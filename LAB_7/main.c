...
void drawPrism(float radius, float height, int sides) {
    ...
    for (int i = 0; i <= sides; ++i) {
        ...
        float x = cos(angle);
        float y = sin(angle);
        float nx = cos(angle + angleStep * 0.5);
        float ny = sin(angle + angleStep * 0.5);
...
void ShowWord() {
    ...
    DrawArrowHead(0, 1, 1, 1, 0, 0, 0, 2, -5);
    DrawArrowHead(1, 0, 1, 0, 1, 0, 214, 2, 2455);
    DrawArrowHead(0, 0, 2, 0, 0, 1, 0, 1, 0);
